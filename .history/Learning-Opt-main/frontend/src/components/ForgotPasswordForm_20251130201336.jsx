...
export default function ForgotPasswordForm({ onBackToLogin }) {
  ...
  const handleSendCode = (e) => {
    ...
    // Mock mode: pretend code was sent
    setCodeSent(true);
  };
  ...
  return (
    <div className="max-w-md w-full p-4 text-white-400">
      {!codeSent ? (
        <form onSubmit={handleSendCode} className="space-y-6">
          ...
          <button
            type="submit"
            className="w-full bg-zinc-300 hover:bg-blue-300 transition p-4 rounded-md font-semibold"
          >
            Send Code
          </button>
          ...
          <button
            ...
            className="w-full text-center text-white-400 hover:underline mt-4"
          >
            Back to Login
          </button>
        </form>
        ...
        <div className="space-y-6">
          ...
          <button
            ...
            className={`w-full p-4 rounded-md font-semibold transition ${
              code.some(d => d === '') 
                ? 'bg-violet-400/60 cursor-not-allowed' 
