...
function DetectionLogs() {
  ...
  const [bypassedRows, setBypassedRows] = useState([]);

  useEffect(() => {
    const loadLogs = async () => {
      const sensor = await axios.get("http://localhost:3000/api/device/logs/SENSOR");
      const bypass = await axios.get("http://localhost:3000/api/device/logs/BYPASS");

      setSensorRows(sensor.data.map(l => `${l.message} (${l.created_at})`));
      setBypassedRows(bypass.data.map(l => `${l.message} (${l.created_at})`));
    };
    ...
    <div className="relative min-h-screen flex flex-col overflow-hidden text-white">
      ...
      <main className="relative z-10 flex-1 p-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <section className="bg-white/10 backdrop-blur-xl mt-3 rounded-2xl shadow-xl overflow-hidden flex flex-col">
